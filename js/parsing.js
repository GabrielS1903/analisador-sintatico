...
function doIteracao(){
	...
	if(first_entrada == "$" && last_pilha == "$"){
		it = new Iteracao(id, pilha, entrada, `<span class='text-success'>Aceito em ${id} iterações</span>`);
		status = "done";
	}else if(first_entrada == last_pilha){
		it = new Iteracao(id, pilha, entrada, "Ler " + last_pilha);
		entrada = entrada.substring(1);
		pilha = pilha.slice(0, -1);
	}else if(first_entrada != "$" && last_pilha == "$"){
		it = new Iteracao(id, pilha, entrada, `<span class='text-danger'>Erro em ${id} iterações</span>`);
		status = "done";
	}else{
...
function drawTable(){
	...
	iteracoes.forEach(function (value) {
...
function drawFita(){
	...
	for (var i = 0; i < entrada.length; i++) {
		...
		if(i == 0){
			$("#table-fita").append("<div class='icon-fita'><img class='icon-image' src='imagem/up-arrow.png'></div>");
		}
 	}
}
	...
	while(!done){
		for(var i=0; i < sentence.length; i++){
			...
			if (character == character.toUpperCase()) {
				...
				let aux = textArray[randomNumber];
